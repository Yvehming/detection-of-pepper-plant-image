...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        plt.plot(hist)
        plt.show()
        cv2.imwrite("0.jpg", bin_img)
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))  # 定义结构元素
        erode = cv2.morphologyEx(bin_img, cv2.MORPH_ERODE, kernel)
        erode = cv2.morphologyEx(erode, cv2.MORPH_ERODE, kernel)
        cv2.imwrite("1.jpg", erode)
        # erode = cv2.morphologyEx(erode, cv2.MORPH_ERODE, kernel)
        # cv2.imwrite("2.jpg", erode)
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (4, 4))  # 定义结构元素
        dilate = cv2.morphologyEx(erode, cv2.MORPH_DILATE, kernel)
        cv2.imwrite("2.jpg", dilate)
        # dilate = cv2.morphologyEx(dilate, cv2.MORPH_DILATE, kernel)
        # cv2.imwrite("4.jpg", dilate)
        # opening = cv2.morphologyEx(closing, cv2.MORPH_OPEN, kernel)
        ...
        cv2.circle(contour_img, (bottommost[0], bottommost[1]), 10, (225, 0, 0), 2)
